  ...
  const K = {
    ...
    spot(parent, x, y, z, tx, ty, tz, color, intensity, o) {
      ...
      if (o.shadow !== false) {
        ...
        l.shadow.bias = o.bias != null ? o.bias : -0.0006;   // window-depth units: -0.0006 is ~24 stage units 8 m from the light, too lenient for a sill or shelf against a wall (a lit strip under it)
        l.shadow.normalBias = o.normalBias != null ? o.normalBias : 0.02;   // o.normalBias: 0 where a shelf or sill meets a wall — the default pushes the shadow's start off the contact line, a lit crack under it
        l.shadow.radius = o.radius != null ? o.radius : 5;   // PCFSoft in r185 honours the blur radius: with a wide cone the texels are big, and 5 of them smear a thin shadow over half a door
        l.shadow.camera.near = 0.15; l.shadow.camera.far = (o.far || 2400) * 0.01;  // metres, not stage px
        if (o.focus != null) l.shadow.focus = o.focus;   // shadow-map fov = 2·angle·focus: a wide cone with focus 1 spends the whole map on empty air, so texels get huge and every attached box (handle, band) smears a false shadow down the surface
      }
      return l;
    },
