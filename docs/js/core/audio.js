...
(function () {
  ...
  const LOOPS = {
    ...
    /** the dream's air, "wind in crayons": sparse glassy wind-chime notes (a pentatonic scale, each with a faint
        wobble in pitch like the boiling picture), a whisper of air that sways slowly from ear to ear like a tide,
        and now and then a very distant soft rumble, a storm past the edge of the dream.
        set(v), v in 0..1: how awake the air is — low in the rooms (rare, dull notes), high in the garden (more often, brighter) */
    chimes() {
      const bus = gain(1); bus.connect(master);
      // --- the whisper: a band of air panned slowly left-right, breathing on a second, unrelated clock
      const n = noise(), f = filt('bandpass', 820, 0.7), wg = gain(0.0001), pan = ctx.createStereoPanner ? ctx.createStereoPanner() : gain(1);
      const swayL = osc('sine', 1 / 13), swayG = gain(0.7); chain(swayL, swayG); if (pan.pan) swayG.connect(pan.pan);
      const brL = osc('sine', 1 / 8.3), brG = gain(0.35); chain(brL, brG); const wBase = gain(1); brG.connect(wBase.gain);   // breath: 0.65..1.35 of the whisper's level
      chain(n, f, wg, wBase, pan, bus); n.start(); swayL.start(); brL.start();
      // --- the chimes
      const NOTES_LOW = [392, 440, 523.3, 587.3, 659.3], NOTES_HIGH = [784, 880, 1046.5, 1174.7];   // C-major pentatonic, G4 up to D6
      let v = 0.3, timer = null, rumbleTimer = null, alive = true;
      const ping = () => {
        if (!alive) return;
        const t = ctx.currentTime + 0.01;
        const pool = Math.random() < v * 0.7 ? NOTES_HIGH : NOTES_LOW;
        const f0 = pool[Math.floor(Math.random() * pool.length)] * U.rand(0.994, 1.006);
        const dur = U.rand(2.2, 3.8), peak = (0.028 + v * 0.03) * U.rand(0.7, 1.1);
        const o = osc('sine', f0), g = gain(0.0001);                                  // the fundamental
        const o2 = osc('sine', f0 * 2.76), g2 = gain(0.0001);                         // one glassy, inharmonic partial, fading sooner
        const wob = osc('sine', U.rand(4.5, 6.5)), wobG = gain(f0 * 0.004); chain(wob, wobG); wobG.connect(o.frequency); wobG.connect(o2.frequency);   // the boil of the picture, in the pitch
        const p = ctx.createStereoPanner ? ctx.createStereoPanner() : gain(1); if (p.pan) p.pan.value = U.rand(-0.7, 0.7);
        env(g, t, 0.006, dur, peak); env(g2, t, 0.004, dur * 0.45, peak * 0.22);
        chain(o, g, p, bus); chain(o2, g2, p); o.start(t); o2.start(t); wob.start(t);
        const end = t + dur + 0.1; o.stop(end); o2.stop(end); wob.stop(end);
        if (Math.random() < 0.28) setTimeout(ping, U.rand(180, 420));                  // sometimes two notes touch
        else timer = setTimeout(ping, (1.6 + (1 - v) * 7) * U.rand(0.5, 1.6) * 1000);          // rooms: a note every 3-14 s; garden: every 1-3 s
      };
      const rumble = () => {
        if (!alive) return;
        const t = ctx.currentTime + 0.01, rn = noise(), rf = filt('lowpass', 140, 0.8), rg = gain(0.0001);
        const rise = U.rand(1.6, 2.6), fall = U.rand(2.5, 4.5);
        rg.gain.setValueAtTime(0.0001, t); rg.gain.linearRampToValueAtTime(0.09 * U.rand(0.6, 1), t + rise); rg.gain.exponentialRampToValueAtTime(0.0001, t + rise + fall);
        rf.frequency.setValueAtTime(90, t); rf.frequency.linearRampToValueAtTime(170, t + rise); rf.frequency.linearRampToValueAtTime(70, t + rise + fall);
        chain(rn, rf, rg, bus); rn.start(t); rn.stop(t + rise + fall + 0.2);
        rumbleTimer = setTimeout(rumble, U.rand(28, 70) * 1000);
      };
      timer = setTimeout(ping, U.rand(700, 2200));
      rumbleTimer = setTimeout(rumble, U.rand(9, 30) * 1000);
      const apply = () => { wg.gain.setTargetAtTime(0.011 + v * 0.014, ctx.currentTime, 1.2); f.frequency.setTargetAtTime(700 + v * 400, ctx.currentTime, 1.5); };   // about a light garden breeze: a whisper, but one you can hear
      apply();
      return {
        set(x) { v = U.clamp(x == null ? 0.3 : x, 0, 1); apply(); },
        stop() {
          alive = false; clearTimeout(timer); clearTimeout(rumbleTimer);
          bus.gain.setTargetAtTime(0.0001, ctx.currentTime, 0.9);
          setTimeout(() => { try { n.stop(); swayL.stop(); brL.stop(); bus.disconnect(); } catch (e) {} }, 4500);
        },
      };
    },
  };
  ...
  CH.audio = {
    ...
    loop(name) {
      // the silent stand-in has to answer everything a real loop answers, `bees().voice` included: the epilogue's bees
      // call it every frame, and a missing method there throws inside their tick, which kills the tick and freezes them
      // off the edge of the screen — with the sound turned off, nobody would ever see a bee
      const silent = { set() {}, stop() {}, voice() {} };
      if (!soundOn() || !ensure()) return silent;
      try {
        const h = LOOPS[name]();
        liveLoops.push(h);
        return h;
