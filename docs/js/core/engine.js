...
(function () {
  ...
  const E = {
    ...
    boxW: 0, boxH: 0,
    walkToken: 0,
    crossing: null,   // the promise of a link crossing under way (a slide, a flight of hops): a newer walk waits for it to end
    hotspots: [],
    ...
    async go(sceneId, spotName, opts) {
      ...
      if (!def.noHero) {
        ...
        CH.hero.place(spot.x, E.platY(spot.plat), spot.plat, E.platZ(spot.plat, spot.x));
    ...
    // ---- shadows: one rule for every scene instead of numbers tuned per lamp ----
    // 1. Shadow maps are drawn from the faces the camera sees. three.js's default draws BACK faces, so for anything resting
    //    on a surface the map holds its underside, flush with that surface, and light leaks through at every contact (a lit
    //    strip under a sill, a ring under a bowl, detached blobs under magnets). A scene may set shadowBackFaces to keep the default.
    // 2. Every shadow-casting light gets its shadow camera fitted to the geometry that receives shadows (a spot's map used to
    //    cover its whole cone, most of it empty air, so texels were huge and every attached box smeared a false shadow down
    //    the surface below it). From the texel size at the light's typical distance it then derives the blur radius
    //    (SHADOW_BLUR m of penumbra) and the depth bias (SHADOW_BIAS m of tolerance, at least twice the blur: PCF self-shadows
    //    when the kernel reach on a steep surface exceeds the bias; contacts open up when the bias exceeds the occluder's gap).
    //    A light may opt out with userData.noShadowFit.
    fitShadows(def) {
      const back = !!def.shadowBackFaces;
      const SHADOW_BLUR = 0.08, SHADOW_BIAS = 0.16;
      // the receivers, mesh by mesh: each one's bounding box, clamped to the part of the stage the camera can see (metres;
      // a wall running up past the screen top or a floor running out past its bottom must not widen the map), and its 8
      // corners — the corners of the union box would be phantom points where nothing stands
      // def.shadowBox (metres) widens that window for a scene the camera sees more of than a room: the garden's fence
      // stands far back, where the frame is much wider than it is at the walking line, and every post must still cast
      const sb = def.shadowBox;
      const lo = new T.Vector3().fromArray(sb && sb.lo ? sb.lo : [-9, -5, -4.5]);
      const hi = new T.Vector3().fromArray(sb && sb.hi ? sb.hi : [9, 5.1, 5]);
      const box = new T.Box3(), tmp = new T.Box3();
      const corners = [];
      E.root.traverse((o) => {
        if (!o.isMesh) return;
        const mats = Array.isArray(o.material) ? o.material : [o.material];
        mats.forEach((m) => { if (m) m.shadowSide = back ? null : m.side; });
        if (!o.visible || !o.receiveShadow || (o.userData && o.userData.noHit)) return;
        if (mats.some((m) => m && (m.visible === false || (m.transparent && m.opacity <= 0.05)))) return;
        tmp.setFromObject(o);
        if (tmp.isEmpty()) return;
        tmp.min.max(lo); tmp.max.min(hi);
        if (tmp.min.x > tmp.max.x || tmp.min.y > tmp.max.y || tmp.min.z > tmp.max.z) return;   // entirely off the visible stage
        box.union(tmp);
        for (let i = 0; i < 8; i++) corners.push(new T.Vector3(i & 1 ? tmp.max.x : tmp.min.x, i & 2 ? tmp.max.y : tmp.min.y, i & 4 ? tmp.max.z : tmp.min.z));
      });
      if (box.isEmpty()) return;
      const lightPos = new T.Vector3(), tgt = new T.Vector3(), axis = new T.Vector3(), v = new T.Vector3(), up = new T.Vector3(0, 1, 0);
      const lights = [];
      E.root.traverse((l) => { if (l.isLight && l.castShadow && l.shadow && !(l.userData && l.userData.noShadowFit)) lights.push(l); });
      lights.forEach((l) => {
        const sh = l.shadow;
        l.updateWorldMatrix(true, false);
        l.getWorldPosition(lightPos);
        // the reference distance is the FARTHEST receiver: a perspective light's depth tolerance grows with distance squared, so
        // fixing it at the far end keeps every contact in the room within SHADOW_BIAS (nearer surfaces get a tighter, still safe, one)
        // (for a spot only receivers inside its cone count: a desk lamp must not be tuned for the far corner of a room it never lights)
        let dRef = 0, need = 0;
        if (l.isSpotLight) {
          l.target.updateWorldMatrix(true, false);
          l.target.getWorldPosition(tgt);
          axis.copy(tgt).sub(lightPos).normalize();
          corners.forEach((c) => {
            v.copy(c).sub(lightPos);
            const d = v.length();
            if (d < 1e-3) return;
            const ang = Math.acos(Math.min(1, Math.max(-1, v.dot(axis) / d)));
            if (ang <= l.angle) { dRef = Math.max(dRef, d); need = Math.max(need, ang); } else if (ang < l.angle + 0.35) { need = l.angle; }
          });
          if (!dRef) corners.forEach((c) => { dRef = Math.max(dRef, c.distanceTo(lightPos)); });
        } else corners.forEach((c) => { dRef = Math.max(dRef, c.distanceTo(lightPos)); });
        dRef = Math.max(dRef, 0.5);
        let texel, biasPerMetre;   // texel: metres per shadow-map texel at dRef; biasPerMetre: window-depth bias for one metre of tolerance there
        if (l.isSpotLight) {
          need = Math.min(l.angle, need + 2 * Math.PI / 180);
          sh.focus = Math.max(0.2, need / l.angle);
          texel = 2 * dRef * Math.tan(need) / sh.mapSize.x;
          if (texel > 0.04 && sh.mapSize.x < 2048) {   // a wide cone on a small map: texels of 4+ units alias every thin shadow — one step up, before the map is first drawn
            sh.mapSize.set(2048, 2048);
            if (sh.map) { sh.map.dispose(); sh.map = null; }
            texel = 2 * dRef * Math.tan(need) / sh.mapSize.x;
          }
          biasPerMetre = sh.camera.near / (dRef * dRef);   // perspective window depth: d(z)/d(dist) = near / dist² when far >> near
        } else if (l.isDirectionalLight) {
          l.target.updateWorldMatrix(true, false);
          l.target.getWorldPosition(tgt);
          const view = new T.Matrix4().lookAt(lightPos, tgt, up).invert();
          const lo = new T.Vector3(Infinity, Infinity, Infinity), hi = new T.Vector3(-Infinity, -Infinity, -Infinity);
          corners.forEach((c) => { v.copy(c).sub(lightPos).applyMatrix4(view); lo.min(v); hi.max(v); });
          const cam = sh.camera, pad = 0.3;
          cam.left = lo.x - pad; cam.right = hi.x + pad; cam.bottom = lo.y - pad; cam.top = hi.y + pad;
          cam.near = Math.max(0.05, -hi.z - 1); cam.far = -lo.z + 1;
          cam.updateProjectionMatrix();
          texel = Math.max((cam.right - cam.left) / sh.mapSize.x, (cam.top - cam.bottom) / sh.mapSize.y);
          biasPerMetre = 1 / (cam.far - cam.near);          // orthographic window depth is linear across near..far
        } else if (l.isPointLight) {
          texel = 2 * dRef / sh.mapSize.x;                    // six 90° faces
          biasPerMetre = 1 / (sh.camera.far - sh.camera.near);   // point shadows store linear distance, normalised to near..far
        } else return;
        sh.radius = U.clamp(Math.round(SHADOW_BLUR / texel), 1, 6);
        const tol = Math.max(SHADOW_BIAS, 2 * sh.radius * texel);
        sh.bias = -tol * biasPerMetre;
        l.userData.shadowFit = { focus: sh.focus, radius: sh.radius, bias: sh.bias, texel: +texel.toFixed(4), dRef: +dRef.toFixed(2), tol: +tol.toFixed(3) };
      });
    },
    ...
    /** the depth a platform's walking line lies at (0: the front line of the room) — at x, when the line bends in depth */
    platZ(id, x) {
      const p = E.inst && E.inst.platforms.find((p) => p.id === id);
      if (!p) return 0;
      if (p.zs && p.zs.length) return E.zsAt(p.zs, x != null ? x : CH.hero.x);
      return p.z ? p.z : 0;
    },
    /** a walking line that bends in depth: zs = [[x, z], ...] in x order, held flat past either end */
    zsAt(zs, x) {
      if (x <= zs[0][0]) return zs[0][1];
      for (let i = 1; i < zs.length; i++) if (x <= zs[i][0]) { const a = zs[i - 1], b = zs[i]; return a[1] + (b[1] - a[1]) * ((x - a[0]) / ((b[0] - a[0]) || 1)); }
      return zs[zs.length - 1][1];
    },
    /** the depth the hero should be at while rolling along a platform, or null when its line is flat (then his depth is left alone) */
    platZAt(id, x) {
      const p = E.inst && E.inst.platforms.find((p) => p.id === id);
      return p && p.zs && p.zs.length ? E.zsAt(p.zs, x) : null;
    },
    ...
    async walkTo(tx, tplatId) {
      ...
      // A walk already under way ends here, not merely at its next check: its tweens would otherwise keep driving him
      // toward the old target underneath this walk, and take over the moment this one finishes first (he would dash
      // off to where the old click pointed). A link crossing — a slide, a flight of hops — is seen through to its far
      // end first, so he is never left in mid-air between two platforms; a plain roll is cut short on the spot. The
      // old walk then gets its turn to notice and return before this one starts (its roll loop stops, ours starts).
      // Only the roll tweens ('walk') are cut: a blink or a squash in flight is none of the walk's business.
      if (E.crossing) { await E.crossing; if (!alive()) return false; }
      tw.kill('walk');
      ...
      // off the walking line at the start (a scene stood him deeper in the room, up at the boots on the stairs): the roll
      // itself merges him back onto the line diagonally. Unless the scene says the way out of this spot is straight out to
      // the line first, round a corner: def.leaveLine(hero, targetX, platform) === 'out'
      const lineZ = E.platZAt(hero.plat, hero.x);
      if (lineZ != null && Math.abs((hero.z || 0) - lineZ) > 1 && E.def.leaveLine && E.def.leaveLine(hero, tx, tplatId) === 'out') {
        if (!(await hero.rollDepth(lineZ, alive))) return false;
        if (!alive()) return false;
      }
      ...
      for (const step of path) {
        ...
        const fromZ = step.forward ? (l.az != null ? l.az : E.platZ(l.a, l.ax)) : (l.bz != null ? l.bz : E.platZ(l.b, l.bx));
        const toZ = step.forward ? (l.bz != null ? l.bz : E.platZ(l.b, l.bx)) : (l.az != null ? l.az : E.platZ(l.a, l.ax));
        ...
        // the crossing runs to its far end whatever is clicked meanwhile: a newer walk waits for it (see the top)
        E.crossing = (async () => {
          ...
          else if (type === 'drop') await hero.dropTo(toX, toY, { z: toZ });
          else if (type === 'hops') await hero.stairHops(toX, toY, l.steps || 4, { z: toZ });
          else if (type === 'float') await hero.floatTo(toX, toY);
          else if (type === 'custom') return (await l.run(step.forward)) !== false;
          else await hero.hopTo(toX, toY, { z: toZ, h: l.h, dur: l.dur });   // a link may ask for its own arc (a long hop over a sink)
          return true;
        })();
        let crossed;
        try { crossed = await E.crossing; } finally { E.crossing = null; }
        if (!crossed) return false;

        hero.plat = toPlat;
        if (!alive()) return false;
        const platZ = E.platZ(toPlat, hero.x);
    ...
    hot(obj, opts) {
      ...
      E.hotspots.push(rec);
      rec.fire = (held, at) => fireHotspot(rec, held, at);
      return rec;
    },
    unhot(obj) {
  ...
  async function fireHotspot(rec, selected, at) {
    const opts = rec.opts;
    ...
    try {
      ...
      try {
        if (selected) {
          ...
          await opts.act(ctx, at);   // at: the click — stage units at the front plane, plus cx/cy on screen (undefined when fired some other way)
        } else if (opts.look) {
  ...
  function stageInputSetup() {
    ...
    canvas.addEventListener('pointerdown', (ev) => {
      ...
      if (rec) { hideLabel(); rec.fire(held, Object.assign(E.toStage(ev.clientX, ev.clientY), { cx: ev.clientX, cy: ev.clientY })); return; }   // a hotspot may care where on it the click fell (a flight of stairs: up or down); cx/cy let it re-read the click at its own depth
      ...
      if (!target) return;
      clickRipple(p.x, p.y);   // a plain click mark: the ring right where the click was, wherever that is
  ...
  /** the click ring: a flat mark facing the camera at the clicked point, drawn over everything (it is cursor feedback, not a thing in the room) */
  function clickRipple(x, y) {
    const geo = new T.RingGeometry(5, 7, 32);
    const mat = new T.MeshBasicMaterial({ color: '#ffb454', transparent: true, opacity: 0.9, depthWrite: false, depthTest: false, fog: false });
  ...
  function makeCameraRig() {
    const cam = E.camera;
    const base = { x: 800, y: 400, z: 1590, tx: 800, ty: 480, fov: 32, follow: 0.1, parallax: 1, flat: false, dolly: null };
    const R = {
      base, mouse: { x: 0, y: 0 }, m: { x: 0, y: 0 }, follow: { x: 0 }, kick: 0, push: 0, t: 0,
      shake: 0, drift: 1, dz: 0,
      setScene(o) {
        Object.assign(base, { x: 800, y: 400, z: 1590, tx: 800, ty: 480, fov: 32, follow: 0.1, parallax: 1, flat: false, dolly: null }, o);
        R.dz = 0;
      },
      enter() { R.kick = 1; },
      ...
      update(dt) {
        ...
        // dolly: { ref, k } — as the hero goes deeper than the reference line the camera follows him in, so he keeps his size and what he walks up to grows
        const dollyOn = base.dolly && hero.attached && (!base.dolly.plats || base.dolly.plats.includes(hero.plat));   // plats: only while he is on these platforms
        // the depth that drives it is the walking line's own at his x (not his momentary depth): rolling in to the foot of a link, or back out, moves the camera not at all
        const lineZ = dollyOn ? E.platZAt(hero.plat, hero.x) : null;
        let dz = dollyOn ? ((lineZ != null ? lineZ : (hero.z || 0)) - base.dolly.ref) * (base.dolly.k != null ? base.dolly.k : 1) : 0;
        // min / max clamp the travel: max: 0 lets the camera go in after him but never back off when the line only steps out for clearance
        if (dollyOn && base.dolly.min != null) dz = Math.max(base.dolly.min, dz);
        if (dollyOn && base.dolly.max != null) dz = Math.min(base.dolly.max, dz);
        R.dz += (dz - R.dz) * Math.min(1, dt * 1.6);
        ...
        const cz = base.z + R.dz + kick * 110 - R.push * 70;
