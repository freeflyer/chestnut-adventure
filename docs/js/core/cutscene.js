...
(function () {
  ...
  const C = {
    ...
    async play(script, opts) {
      ...
      // A script belongs to the scene it started in. Killing a tween RESOLVES it (that is how a skip unwinds), so a scene
      // change on its own would not stop the script: it would run on against a stage whose layers have just been emptied,
      // and the first object it touched would throw. So the generation counts as an abort, and the script unwinds cleanly.
      const gen0 = CH.engine.gen;
      const guard = (v) => { if (C.aborted || CH.engine.gen !== gen0) throw SKIP; return v; };
      ...
      const ctx = {
        ...
        t: CH.t,
        dead: () => C.aborted || CH.engine.gen !== gen0,
