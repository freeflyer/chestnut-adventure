...
(function () {
  ...
  const BOOT_X = stepX(1) - 46;   // where he stands up at the boots: in front of the first riser (his husk is 30 across, the spikes reach past)
  const BOOT_Z = -175;            // in the middle of the flight's depth, where the boots lie
  const WALL = -330;
  ...
  CH.defScene('hallway', {
    ...
    // the camera goes in with him as his line turns toward the vent in the left corner (same rule as the study): ref = the line's usual depth
    camera: { x: 800, y: 400, z: 1590, tx: 800, ty: 480, follow: 0.1, dolly: { ref: 0, k: 1.8, plats: ['floor'], max: 0 } },   // max 0: in toward the vent only, the line's step out in front of the stairs is clearance, not a camera cue

    platforms: [
      // the walking line runs at z 0; from x 410 it goes in a long diagonal to the wall so he walks right up to the open vent (its face
      // is at z -290; his back sits 13 in front of it at z -240). The radiator in the corner ends at x 56, out of his way
      // Past x 1000 it steps out to z 60: the stair treads reach z -20, the banister -38..-10 and the cat flap +5 — at z 0 his far
      // side sank into them
      { id: 'floor', x1: 90, x2: 1580, y: FLOOR_Y, zs: [[90, -240], [190, -240], [410, 0], [930, 0], [1000, 60], [1580, 60]] },
      { id: 'step3', x1: stepX(3) + 6, x2: stepX(3) + STEP_W - 6, y: stepTop(3), z: -170 },   // the third tread, at the treads' middle depth: where the umbrella ends, and where he can stop halfway up
    ...
    // stood up at the boots he is off the walking line, in the flight's depth. Leaving to the LEFT is a diagonal back onto
    // the line; leaving to the RIGHT the flight itself is in the way, so it is straight out to its front corner first
    leaveLine: (hero, tx) => (hero.x > 700 && hero.x < stepX(1) + 10 && (hero.z || 0) < -40 && tx > hero.x ? 'out' : 'diag'),
    spots: {
      fromVent: { x: 210, plat: 'floor' },
      center: { x: 750, plat: 'floor' },
      fromLiving: { x: 1480, plat: 'floor' },
      fromKitchen: { x: 1160, plat: 'floor' },
      fromStairs: { x: 1520, plat: 'landing' },   // down from the corridor: enter() walks him down the flight and off the umbrella
      stairsFoot: { x: 960, plat: 'floor' },      // where that descent ends (the checkpoint moves here once he is down)
    },
    ...
    build(api) {
      ...
      // each step = the block under it + a 6-thick tread board on top. The board's nosing stands 2 proud of the riser and
      // of the flight's side, so no face of one lies in a face of the other: the camera never quite stands still (it
      // drifts), and two faces in one plane shimmer against each other
      for (let k = 1; k <= STEPS; k++) {
        K.box(stepX(k), stepTop(k) + 6, 1900 - stepX(k), STEP_H - 6, 310, treadMat, stairsG, { z: -175 });
        K.box(stepX(k) - 2, stepTop(k), 1902 - stepX(k), 6, 314, '#7c5836', stairsG, { z: -175 });
        // carpet runner: the tread strip wraps over the nosing (starts 3 before the riser, 1 above the board) and the
        // riser strip hangs from its underside, so the board's corner is inside the carpet and no two faces share a plane
        K.box(stepX(k) - 3, stepTop(k) - 1, STEP_W + 3, 5, 200, '#8a4a52', stairsG, { z: -175 });
        K.box(stepX(k) - 3, stepTop(k) + 4, 5, STEP_H - 4, 200, '#7c414a', stairsG, { z: -175 });
      }
      ...
      // ---------- under-stairs: the kitchen flap + Vera ----------
      // the WORKING cat flap — the one thing under the stairs that must catch the eye — is cut straight into the flight's
      // side (the step boxes' front face, z -20): its little door does not quite reach the sill, and the kitchen's warm
      // light leaks through the gap under it and faintly onto the boards in front
      const kflapG = K.g(main, { z: -14 });   // the frame sits 2 into the face and 6 proud of it
      ...
      K.glow(main, 1131, 798, -2, 30, '#ffb454', 0.1);    // a soft haze at the sill
      K.point(L, 1131, 792, 16, '#ffcf7a', 1.3, 240);     // and a faint spill onto the floor in front
      ...
      const wSpan = 90, wDrop = 130;   // the fan: the outer spokes reach wSpan to each side at wDrop below the anchor
      for (let i = 0; i < 5; i++) seg([[wx, wy], [wx - wSpan + i * (wSpan / 2), wy + wDrop]]);
      // each cross thread is strung from one outer spoke across to the other, so it hangs wider than the one above it
      // (the span comes off the spokes themselves, so the two can never drift apart), and bows by the same share of its own width
      for (let r = 1; r <= 3; r++) {
        const pts = [], drop = 34 * r, half = wSpan * (drop / wDrop);   // it ends exactly on the outer spoke, where it would be knotted on
        for (let t = 0; t <= 8; t++) { const u = t / 8; pts.push([U.lerp(wx - half, wx + half, u), wy + drop + Math.sin(u * Math.PI) * half * 0.15]); }
        seg(pts);
      }
      ...
      api.hot(ventG, {
        ...
        act: async () => {
          ...
          await api.cut(async (ctx) => {
            await ctx.run(api.hero.rollTo(170, () => true));   // square in front of the vent, on the line's deep end (z -240)
            api.hero.face(-1);
            await ctx.w(160);
            // and in: one hop straight into the opening (the dark box behind the vent runs z -350..-310), as in the study
            await ctx.run(api.hero.hopTo(170, FLOOR_Y, { h: 24, dur: 380, z: -335 }));
            ctx.sfx('slide', false);
          }, { cinema: false, skippable: false });
      ...
      const climb = async () => {
        ...
        try {
          ...
          if (!st.has('gotBrolly')) {
            await api.think(st.has('veraMet') ? 'c3.up.notyet' : 'c3.up.unsure');   // before he has met Vera he cannot quote her: he just feels it is not time to go up yet
            return;   // and he stays up there: coming down is the player's click, like anywhere else (a scripted walk back down used to end under the landing, on Vera's patch, and to override whatever the player clicked meanwhile)
          }
      ...
      // With the umbrella down the flight is terrain with two stops on it: the third tread (where the umbrella ends,
      // halfway up) and the landing. A click goes to the stop its tread belongs to — the umbrella and treads 1–3 to the
      // third, 4–8 to the landing — and a click on the part he is already standing on takes him one level down, so from
      // the landing the flight is the way back to the middle, and from the middle the lower treads are the way to the
      // floor. Only the landing itself (h.upstairs) means "go upstairs". The click is read at the treads' own depth.
      const onFlight = async (at) => {
        if (!at) { await climb(); return; }
        const p = CH.engine.toStage(at.cx, at.cy, -177);
        const k = Math.floor((p.x - STEP_X0) / STEP_W) + 1;   // which tread: tread k spans stepX(k)..stepX(k + 1) (the umbrella comes out below 1)
        let dest = k <= 3 ? 'step3' : 'landing';
        if (dest === api.hero.plat) dest = dest === 'landing' ? 'step3' : 'floor';
        await api.walkTo(dest === 'landing' ? 1540 : dest === 'step3' ? stepX(3) + 30 : 900, dest);
      };
      ...
      api.hot(umbG, {
        ...
        act: async (ctx, at) => {
          if (st.has('umbrellaRamp')) { await onFlight(at); return; }
      ...
      // the way up is marked like the way out to the living room: the same little arrow, over the landing, pointing up and
      // bobbing up. No pad of its own: the landing's pad below is the click, and a pad turned upward would lie over the top treads
      CH.props.exitMark(api, 1575, 200, 'up', 0);   // at the front plane like the other marks: at the landing's depth the perspective pulled it in toward the middle of the frame, down-left of the landing
      const landingZone = K.pad(1480, stepTop(8) - 200, 110, 210, main, { d: 300 });   // up to y 184 (was 244): the arrow above is part of the click
      api.hot(landingZone, {
        id: 'h.upstairs',
      ...
      api.hot(stairsHot, {
        id: 'h.stairs',
        near: null,   // the way in is the act's own (a diagonal from the left, round the corner from the right), not a floor spot
        act: async (ctx, at) => {
          if (st.has('umbrellaRamp')) { await onFlight(at); return; }
          const n = st.bumpClick('h.stairs');
          const tok = CH.engine.walkToken, alive = () => CH.engine.walkToken === tok;   // a floor click meanwhile takes over, as with any walk
          const atBoots = Math.abs(api.hero.x - BOOT_X) < 40 && (api.hero.z || 0) < -120;   // already stood up at them: a repeat look does not walk
          if (!atBoots) {
            // up to the boots, in the middle of the flight's depth: from the left a diagonal in; from the right the flight is in the
            // way, so along the line to its front corner first, then straight in
            if (api.hero.x > BOOT_X + 40) {
              if (!(await api.hero.rollTo(BOOT_X + 30, alive))) return;
              if (!(await api.hero.rollDepth(BOOT_Z, alive))) return;
            }
            if (!(await api.hero.rollTo(BOOT_X - 30, alive, { z: BOOT_Z }))) return;
            api.hero.face(1);
            if (n === 1) {
              await api.cut(async (ctx) => {   // a hopeful little hop that bonks on the boots
                await ctx.run(api.hero.hopTo(BOOT_X, FLOOR_Y, { h: 60, dur: 420, z: BOOT_Z }));
                ctx.sfx('thud');
                await ctx.run(api.hero.dizzy(700));
              }, { cinema: false, skippable: false });
            } else if (!(await api.hero.rollTo(BOOT_X, alive, { z: BOOT_Z }))) return;
          }
          await api.think(n === 1 ? 'c2.stairs.tall' : 'c2.stairs.tall2');
        },
      });
    },
    ...
    enter(api) {
      const st = api.state;
      if (api.hero.plat === 'landing') {
        // down from the corridor: from the head of the flight he hops down the treads and slides off the umbrella at the end
        api.cut(async (ctx) => {
          await ctx.w(300);
          let ok = false;
          await ctx.run(api.walkTo(960, 'floor').then((v) => { ok = v; }));
          if (ok) { st.data.spot = 'stairsFoot'; st.setCheckpoint('hallway', 'stairsFoot'); }
          else api.hero.place(1420, FLOOR_Y, 'floor', CH.engine.platZ('floor', 1420));   // no umbrella ramp (not possible past chapter 4): set him down at the foot
        }, { cinema: false, skippable: false });
        return;
      }
      if (!st.has('hallFirst')) {
      ...
      if (!st.has('veraMet')) {
        ...
        const un = api.tick(() => {
          ...
          if (api.hero.x > 1150 && api.hero.plat === 'floor') {
            un();
            CH.engine.walkToken++; CH.tw.kill('walk');   // the walk ends here: she is talking to him
            meetVera();
          }
        });
      }
