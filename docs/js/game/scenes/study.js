...
(function () {
  ...
  CH.defScene('study', {
    ...
    // the camera goes in with him as his line turns toward the vent (like the dream kitchen's fridge): ref = the line's usual depth
    camera: { x: 800, y: 400, z: 1590, tx: 800, ty: 480, follow: 0.1, dolly: { ref: 40, k: 1.8, plats: ['floor'] } },

    platforms: [
      // the walking line: a little toward the viewer (z 40); from the left half of the bookshelf (x 950..1180, z -330..-265) it runs in
      // a long diagonal to the wall so he walks right up to the vent grate at an even pace (its face is at z -290; his back sits 13 in
      // front of it at z -240) — along the shelf his back stays in front of its face
      { id: 'floor', x1: 120, x2: 1430, y: 800, zs: [[120, 40], [1020, 40], [1240, -240], [1430, -240]] },
    ],
    links: [],
    spots: {
    ...
    build(api) {
        ...
        act: async () => {
          if (st.has('ventOpen')) {
            await api.cut(async (ctx) => {
              ...
              api.hero.face(1);
              await ctx.w(160);
              // in he goes: he already stands at the wall (line z -240, the grate's face at -290), so one hop straight into the opening —
              // the dark box behind the grate runs z -350..-310, at -335 he sits inside it with his front just behind the grate plane
              await ctx.run(api.hero.hopTo(1242, 800, { h: 24, dur: 380, z: -335 }));
              await ctx.w(160);
            }, { cinema: false, skippable: false });
      ...
      if (st.has('cableDown')) {
        api.hot(cableG, {
          id: 's.cableUp',
          near: { x: 710, plat: 'floor' },   // to the RIGHT of the hanging cable (x ~664..670)
          act: async () => {
            await api.cut(async (ctx) => {
              await ctx.run(api.hero.rollDepth(-29, () => true));   // at the cable's depth, beside it
              api.hero.face(-1);   // facing the cable he holds
              await ctx.run(api.hero.climbUp(710, 356));
            }, { cinema: false, skippable: false });
