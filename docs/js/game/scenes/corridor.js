...
(function () {
  ...
  CH.defScene('corridor', {
    ...
    platforms: [
      // the walking line runs at z 0 and from x 470 bends in a long diagonal to the head of the flight, mid-depth of the well
      // (x 230, z -190): he goes down the stairs from there, not off the floor's edge in front of the balustrade
      { id: 'floor', x1: 220, x2: 1480, y: 800, zs: [[220, -190], [230, -190], [470, 0], [1480, 0]] },
    ],
    links: [],
    spots: {
    ...
    build(api) {
      ...
      P.room(api, {
        ...
        holeTrim: 14,                                                                          // and the baseboard stops at their architraves (14 wide) instead of running across them like a threshold
      ...
      // the nosing along the near edge of the hole: inside the well and flush with the floor, reaching back to the first riser
      // (sitting on the floor, its top lay in the floor's plane and the two shimmered)
      K.box(WX1, 800, WX2 - WX1, 10, 10, K.mat('#2a2018', { rough: 0.9 }), stairsDown, { z: WZ2 - 5 });
      for (let k = 1; k <= 8; k++) {   // the flight: treads stepping down to the left, the runner down the middle
        const tx = WX2 - SW * k, ty = 800 + SH * k, zc = (WZ1 + WZ2) / 2, zd = WZ2 - WZ1 - 20;
        K.box(tx, ty, SW, 10, zd, treadMat, stairsDown, { z: zc });
        K.box(tx + SW - 8, ty - SH, 8, SH, zd, riserMat, stairsDown, { z: zc });   // the riser at the tread's right end: its open face (x tx+60) looks down the flight
        K.box(tx, ty - 1, SW, 5, zd * 0.6, '#8a4a52', stairsDown, { z: zc });
        // the riser's carpet strip sits on that open face (3 proud, 2 in) and starts 1 below the riser's top: it used to sit inside
        // the riser and poke 2 past the well's edge under the floor, its top in one plane with the riser's top and the floor — they shimmered
        K.box(tx + SW - 11, ty - SH + 1, 5, SH - 1, zd * 0.6, '#7c414a', stairsDown, { z: zc });
      }
      ...
      // the way down is marked like the way up in the hall: the same little arrow over the well, pointing down and bobbing down
      CH.props.exitMark(api, 100, 660, 'down', 0);   // below the handrail (it passes y 578 at this x), over the balusters, in front of them
      ...
      // picture
      P.picture(far, 940, 260, 140, 100, (ctx, w, h) => {   // between the doors, clear of the bathroom door's architrave (its frame reaches 8 out)
      ...
      // the pull cord comes out of the hatch's underside: it starts inside the box and hangs within its depth, so its
      // shadow grows out of the hatch's own shadow instead of starting a little below it
      K.cylUp(955, 150, 1.5, 96, K.mat('#8a8296', { rough: 0.7 }), hatchG, { z: 4 });
      K.sphere(955, 156, 6, K.mat('#8a8296', { rough: 0.7 }), hatchG, { z: 4 });
      ...
      api.hot(stairsDown, {
        ...
        act: async () => {
          await api.cut(async (ctx) => {
            // in along the line's diagonal to the head of the flight, then down the treads (one hop each) and out of sight down the well
            await ctx.run(api.hero.rollTo(230, () => true));
            await ctx.run(api.hero.stairHops(-116, 1060, 5, { z: -190 }));
          }, { cinema: false, skippable: false });
