...
(function () {
  ...
  CH.defScene('dreamRoom', {
    ...
    build(api) {
      CH.audio.loop('chimes').set(0.3);   // the dream's air: sparse glassy chimes, a swaying whisper, a far rumble now and then — rare and dull indoors (stops with the scene)
      const st = api.state;
      ...
      const ctip = K.cone(0, 0, 6, 14, '#e2635f', crayonG); K.tr(ctip, { r: -90, x: 37, y: 0 });   // the point continues the body: base flush on its end (x 30), apex at 44, on the axis
  ...
  CH.defScene('dreamHall', {
    ...
    build(api) {
      CH.audio.loop('chimes').set(0.25);   // the dream's air: sparse glassy chimes, a swaying whisper, a far rumble now and then — rare and dull indoors (stops with the scene)
      const st = api.state;
      const far = api.layers.far, mid = api.layers.mid, main = api.layers.main, fg = api.layers.fg, fx = api.layers.fx;
      const P = CH.props;
  ...
  CH.defScene('dreamKitchen', {
    ...
    camera: { x: 800, y: 400, z: 1590, tx: 800, ty: 480, follow: 0.1, dolly: { ref: 150, k: 3, plats: ['floor'] } },   // the camera goes in with him when he walks up to the fridge, three times as far as he does: up close, the cat fills the eye. Floor only: the counter (z 0) must not pull it in

    platforms: [
      { id: 'floor', x1: 140, x2: 1120, y: 800, zs: [[140, 60], [470, 60], [620, 150], [1120, 150]] },   // his walking line: out in the room (z 150; the open fridge door reaches z 95, his spikes 37) but, left of the door, in close to the fridge (z 60)
      { id: 'counter', x1: 710, x2: 1490, y: 516 },
      { id: 'bowl', x1: 1190, x2: 1390, y: 792, z: 150 },     // the bowl's floor, down the middle of the bowl, on the room's walking line: he hops straight in, walks it half-hidden by the walls, hops out
      { id: 'floorR', x1: 1470, x2: 1570, y: 800, z: 150 },
    ],
    links: [
      { a: 'floor', b: 'counter', ax: 760, az: 0, bx: 760, type: 'float' },   // he rolls in to the counter's line (z 0) first, then floats straight up onto it
    ...
    build(api) {
      ...
      // the box of the fridge: a hollow cabinet (back, two sides, top, bottom), open at the front, its depth z -110..90 here —
      // so what the door reveals is really inside it, not hung in front of a solid block
      K.box(250, 220, 240, 580, 12, steel, fridgeG, { z: -100 });   // back wall (z -106..-94 here = -326..-314: clear of the room's wall at -330)
      K.box(250, 220, 12, 580, 200, steel, fridgeG, { z: -10 });    // left wall
      K.box(478, 220, 12, 580, 200, steel, fridgeG, { z: -10 });    // right wall
      K.box(250, 220, 240, 12, 200, steel, fridgeG, { z: -10 });    // top
      K.box(250, 788, 240, 12, 200, steel, fridgeG, { z: -10 });    // bottom
      // the interior (revealed when the door swings): a sea of milk, deep in the cabinet, lit by the fridge's own little lamp
      const insideG = K.g(fridgeG, { z: 0 });
      insideG.visible = st.has('fridgeOpen');
      K.box(262, 232, 216, 556, 4, K.mat('#5f7385', { rough: 0.9 }), insideG, { z: -90 });   // the lining of the back wall: a fridge's inside, not the room's wall — a mid slate blue, dark enough that Biscuit's pale dream-cloud is the brightest thing in there
      K.point(insideG, 370, 250, 50, '#eef4ff', 0.9, 420);   // the fridge lamp, soft: enough to see by, not a white glare behind the cloud
      // the sea of milk fills the cabinet's floor from the back lining (z -92) to near the front, with two rows of waves
      const milkMat = K.mat('#f4f1ea', { rough: 0.6 });
      const milk = [];
      for (let i = 0; i < 9; i++) milk.push(K.sphere(268 + i * 27, 700, 20, milkMat, insideG, { z: -30 }));         // front row
      for (let i = 0; i < 9; i++) milk.push(K.sphere(255 + i * 27, 706, 18, milkMat, insideG, { z: -68 }));         // back row, between the front ones
      K.box(262, 704, 216, 90, 150, milkMat, insideG, { z: -17 });   // z -92..58: right up to the lining, so nothing dark shows behind the waves
      K.rbox(312, 640, 120, 110, 60, 8, K.mat('#e8eef0', { rough: 0.6 }), insideG, { z: 0 }); // carton island, mid-depth: wide enough for the cat to lie across it
      K.rbox(310, 648, 124, 26, 64, 8, K.mat('#5a9e8f', { rough: 0.6 }), insideG, { z: 0 });   // the carton's label band: 2 proud of the carton on every side and 8 below its top, so no face of it lies in a face of the carton (that z-fought and flickered)
      let mt = 0;
      api.tick((dt) => {
        mt += dt;
        milk.forEach((m, i) => (i < 9 ? K.tr(m, { x: 268 + i * 27, y: 700 + Math.sin(mt * 1.6 + i * 0.7) * 8, z: -30 }) : K.tr(m, { x: 255 + (i - 9) * 27, y: 706 + Math.sin(mt * 1.6 + i * 0.7 + 1.1) * 7, z: -68 })));
      });
      // Biscuit, asleep on the island, and his dream-cloud — still well behind the front opening (z 90)
      const dcat = CH.actors.cat(K.g(insideG, { z: 40 }), 388, 640, 0.6);   // a little right of the carton's centre, so his front paw rests on it instead of hanging off the left edge
      void dcat;
      const catCloud = DF.cloud(insideG, 388, 536, 1.125, 40);   // a quarter bigger than the room's clouds, so it is not missed in the dim fridge; above the cat
      // the door face (a group, so it can swing on its hinge at the right)
      const doorFace = K.g(fridgeG, { z: -10 });   // its hinge line sits on the cabinet's front edge (z 90), not 10 in front of it
      ...
      // the door swings on its right edge OUT towards the viewer (ry +80; -80 would swing it back through the cabinet);
      // open, it stands edge-on beside the fridge, and the drawing's glow goes with the face we no longer see
      const setFridge = (open) => {
        if (open) K.tr(doorFace, { ry: 70, ox: 490, oy: 0 });
        else K.tr(doorFace, { ry: 0, ox: 490, oy: 0 });
        drawGlow.visible = !open;
      };
      ...
      const bowlG = K.g(main, { z: 150 });   // centred on the walking line (z 150), well out from the counter (front at z -50): an oval in plan, deep enough (0.55) that his spikes (z +-37) stay inside its front wall even at the ends of his walk (x 1190/1390, where the wall is at z 46-60 from the bowl's centre line)
      {   // one hollow lathe: a flared wall 10 thick, open at the top, a dark floor inside
        ...
        K.tr(bowl, { x: 1290, y: 800, r: 180, sz: 0.55 });
        // the dark floor inside: squashed to the same oval as the bowl (K.cylUp ignores an sz option — as a round disc it
        // stuck 74 out through the front wall and lay along the base as a pale lit strip)
        K.tr(K.cylUp(1290, 792, 118, 2, K.mat('#2c3a4c', { rough: 0.8 }), bowlG, { seg: 40 }), { sz: 0.55 });
      }
      K.label(CH.t('name.cat').toUpperCase(), { size: 28, color: '#d8e4ec', x: 1290, y: 774, z: 80, parent: bowlG });   // just in front of the wall, which at this height is at z 76
          ...
          if (st.has('fridgeHandle')) {
            await api.cut(async (ctx) => {
              ...
              await ctx.tw({ t: 0 }, { t: 1 }, {
                ...
                onUpdate: (k, o) => { K.tr(doorFace, { ry: 70 * o.t, ox: 490, oy: 0 }); drawGlow.visible = o.t < 0.5; },
              });
              ctx.sfx('meow', 0.7);
              await ctx.w(400);
            }, { cinema: false, skippable: false });
      ...
      api.hot(catCloud.g, {
        id: 'dk.catcloud', near: { x: 420, plat: 'floor' },   // right up at the open fridge
        active: () => st.has('fridgeOpen'),
        act: async () => {
          api.hero.face(-1);
  ...
  CH.defScene('dreamGarden', {
    ...
    build(api) {
      CH.audio.loop('chimes').set(0.85);   // the dream's air: sparse glassy chimes, a swaying whisper, a far rumble now and then — awake and bright out here (stops with the scene)
      const st = api.state;
