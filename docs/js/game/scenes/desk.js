...
(function () {
  ...
  CH.defScene('desk', {
    ...
    platforms: [
      // the walking line bends toward the viewer past x 1040 so he passes in front of the tape dispenser (z -50..-10) and the
      // book stack (z -140..20) instead of rolling into them; the desk top runs out to z 230, so z 70 is still well on it
      { id: 'desk', x1: 120, x2: 1330, y: 640, zs: [[120, 0], [1040, 0], [1120, 70], [1330, 70]] },
    ],
    links: [],
    spots: {
    ...
    build(api) {
      ...
      // a hexagonal body: K.rodX already places the mesh at the rod's middle (x 47), so the swapped-in geometry stays centred — a
      // translated one doubled the offset, the body ran 47..141 and swallowed the point and the lead at 94..106
      K.rodX(0, 94, 0, 4.2, K.mat('#e8a13c', { rough: 0.6 }), pencilG).geometry = new T.CylinderGeometry(4.2, 4.2, 94, 6).rotateZ(Math.PI / 2);
      const tip = K.cone(100, 0, 4.2, 12, '#d8c8a8', pencilG);
      K.tr(tip, { r: -90, x: 100, y: 0 });   // the point continues the rod: base flush on its end (x 94), apex at 106 where the lead sits, on the axis
      K.sphere(106, 0, 1.6, '#3a2b22', pencilG);
      K.rbox(-9, -4, 12, 8, 8, 2, '#e2938a', pencilG);   // the eraser cap sticks out 9 past the body's end so it reads as its own thing
      ...
      const descendCable = async () => {
        await api.cut(async (ctx) => {
          ...
          // to the RIGHT of the hanging cable (x ~1402..1410, z 60) and clear of the desk top's end (x 1382): a short hop onto the cable
          // just under the edge, then down beside it at its depth
          await ctx.run(api.hero.hopTo(1452, 672, { z: 60, h: 26, dur: 380 }));
          api.hero.face(-1);   // facing the cable he holds
          await ctx.run(api.hero.climbDown(1452, 850));
        }, { cinema: false, skippable: false });
        st.flag('deskDown');
