        ...
        item: {
          seed: async () => {
            await api.cut(async (ctx) => {
              api.hero.face(1);
              // the tail snaps up toward the pigeon and stays out while the seed leaves its very tip and arcs up to the
              // bird: a light striped seed, big enough to follow against the night wall (it used to be a dark 4-unit dot
              // that left from beside him while the tail only twitched)
              const whip = api.hero.tailWhip(470, 200, { hold: 520 });
              await ctx.w(160);
              const from = api.hero.tailTip();
              const seedEl = K.ellipsoid(0, 0, 5, 8, 3.5, K.mat('#d8ccb0', { rough: 0.6 }), api.layers.fx);
              await ctx.tw({ t: 0 }, { t: 1 }, {
                dur: 560, ease: CH.tw.ease.quadOut,
                onUpdate: (k, o) => K.tr(seedEl, { x: U.lerp(from.x, 512, o.t), y: U.lerp(from.y, 58, o.t) - Math.sin(o.t * Math.PI) * 44, z: U.lerp(from.z, -34, o.t), r: o.t * 420 }),   // to the beak's tip (the bird's origin is at 482/86, its beak reaches 34 right and 29 up of it)
              });
