...
(function () {
  ...
  const hero = {
    ...
    // ------------------------------------------------- movement verbs
    /** roll along the platform to x. opts.z: end at that depth instead of on the walking line, peeling off the line
        diagonally over the last opts.blend (220) units, or going there straight when he is already off the line.
        Starting off the line with no opts.z, he merges back onto it diagonally over the first stretch, never with a jump. */
    async rollTo(tx, alive, opts) {
      if (!root) return false;
      const dist = Math.abs(tx - hero.x);
      if (dist < 2) return true;
      const lineAt = (x) => (CH.engine.platZAt ? CH.engine.platZAt(hero.plat, x) : null);
      const xs = hero.x, z0 = hero.z || 0, l0 = lineAt(xs), offLine = l0 != null && Math.abs(z0 - l0) > 1;   // xs: where the roll starts (hero.x moves)
      const tz = opts && opts.z != null ? opts.z : null, blend = Math.min((opts && opts.blend) || 220, dist);
      const depthAt = (x, u) => {
        const lz = lineAt(x);
        if (tz != null) return (offLine || lz == null) ? U.lerp(z0, tz, u) : U.lerp(lz, tz, U.clamp(1 - Math.abs(tx - x) / blend, 0, 1));
        if (lz == null) return null;
        return offLine ? U.lerp(z0, lz, U.clamp(Math.abs(x - xs) / blend, 0, 1)) : lz;
      };
      hero.face(tx - hero.x);
      ...
      const ok = await tw.to(o, { x: tx, ph: dist / (14 * A.scale) }, {
        dur, ease: tw.ease.quadInOut, group: 'walk',   // the roll itself: a newer walk cuts it short (the 'hero' group — blinks, squashes, the hops and slides of a crossing — is left alone)
        onUpdate: () => {
          hero.x = o.x;
          const u = span ? U.clamp((o.x - x0) / span, 0, 1) : 1;
          const pz = depthAt(o.x, u);   // the walking line (it bends in depth), or the diagonal on or off it
          if (pz != null) hero.z = pz;
    ...
    async climbUp(tx, ty) {
      if (Math.abs(tx - hero.x) > 0.5) hero.face(tx - hero.x);   // a straight climb keeps the facing the scene set (toward the cable)
    ...
    async dropTo(tx, ty, opts) {
      opts = opts || {};
      hero.face(tx - hero.x);
      const x0 = hero.x, y0 = hero.y, z0 = hero.z || 0, tz = opts.z != null ? opts.z : z0;   // opts.z: the floor below lies at another depth
      // stepping off toward another depth: a small lift first, and the move outward runs ahead of the fall, so the body clears the
      // edge before it sinks below the surface it left (without this he fell straight through the counter's corner)
      const lift = tz !== z0 ? 12 : 0;
      await tw.to({ t: 0 }, { t: 1 }, {
        dur: Math.max(300, Math.abs(ty - y0) * 1.1), ease: tw.ease.linear, group: 'hero',
        onUpdate: (k, o) => {
          const fall = o.t * o.t;                       // gravity — the quadIn the whole tween used to have
          const out = 1 - (1 - o.t) * (1 - o.t);        // the outward move leads
          hero.x = U.lerp(x0, tx, lift ? out : fall);
          hero.y = U.lerp(y0, ty, fall) - Math.sin(o.t * Math.PI) * lift;
          hero.z = U.lerp(z0, tz, out);
          A.sy = 1 + fall * 0.15; A.sx = 1 - fall * 0.1;
        },
      });
    ...
    /** the tail snaps out toward a stage point and relaxes after 260 ms (opts.hold: stay out that much longer, e.g. while a thing it flicked leaves its tip) */
    async tailWhip(sx, sy, opts) {
      ...
      await tw.delay(260 + ((opts && opts.hold) || 0), 'hero');
      A.tailTx = null; A.tailTy = null;
    },

    /** where the tail's tip is right now, in stage coordinates: the point a thing flicked by the tail leaves from */
    tailTip() {
      const v = parts.tailTip.getWorldPosition(new T.Vector3());
      return { x: v.x * 100 + 800, y: 450 - v.y * 100, z: v.z * 100 };
    },
